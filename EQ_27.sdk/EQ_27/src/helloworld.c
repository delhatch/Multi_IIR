...
#define NUM_BANDS 27     /* Number of bands (IIR "instances") in the filter. Must be <= 30 */
...
//--------------  END OF LEDS  ---------------------------------------------
	...
	// Note: Never set p > 2.0. Note that high values of p in the low freq bands MAY be unstable
	//       due to overflow in the filter. Full-scale signal may result. You have been warned.
	set_coef( &set[0], 31.5, 0.3);
	set_coef( &set[1], 40, 0.6);
	set_coef( &set[2], 50, 1.0);
	set_coef( &set[3], 63, 1.5);
	set_coef( &set[4], 80, 1.0);
	set_coef( &set[5], 100, 0.5);
	set_coef( &set[6], 125, 0.5);
	set_coef( &set[7], 160, 0.01);
	set_coef( &set[8], 200, 0.01);
	set_coef( &set[9], 250, 0.01);
	set_coef( &set[10], 315, 0.01);
	set_coef( &set[11], 400, 0.01);
	set_coef( &set[12], 500, 0.01);
	set_coef( &set[13], 630, 0.01);
	set_coef( &set[14], 800, 0.01);
	set_coef( &set[15], 1000, 0.5);
	set_coef( &set[16], 1250, 0.01);
	set_coef( &set[17], 1600, 0.01);
	set_coef( &set[18], 2000, 0.01);
	set_coef( &set[19], 2500, 0.01);
	set_coef( &set[20], 3150, 0.01);
	set_coef( &set[21], 4000, 0.5);
	set_coef( &set[22], 5000, 0.5);
	set_coef( &set[23], 6300, 1);
	set_coef( &set[24], 8000, 1.5);
...
void set_coef( struct coefs *this_set, float f0, float p)
{
	...
	//f0 = 1000;
	//% Enter gain. 1.0 = +6dB. -0.5 = -3 dB. 0.1 = +0.6 dB. 2.0 = +12 dB. Keep between +/- 2.0.
	//p = 1.0;
